...
{#if (notice.description && notice.description.trim()) || (notice.files && notice.files.length > 0)}
	<a href="/notice/{notice._id}" class="{isPast ? 'bg-neutral-50 dark:bg-neutral-700 border-neutral-200 dark:border-neutral-700 opacity-75 hover:opacity-90 transition-opacity' : 'bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-600 hover:border-neutral-400 dark:hover:border-neutral-500 transition-colors'} border p-2 sm:p-3 block" aria-label="{notice.subject} {notice.title} 상세 보기">
		<div class="flex items-start justify-between gap-2 sm:gap-4">
			...
			<div class="text-neutral-400 dark:text-neutral-500 text-sm {isPast ? 'opacity-75' : ''}" aria-hidden="true">
				→
			</div>
		</div>
	</a>
